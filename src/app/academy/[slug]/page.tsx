...
export async function generateMetadata({ params }: { params: Promise<{ slug: string }> }): Promise<Metadata> {
  ...
  return {
    ...
    alternates: { canonical: `https://memedesk.co/academy/${slug}` },
    openGraph: {
...
function getRelatedArticles(current: Article, allArticles: Article[], count = 3) {
  const others = allArticles.filter((a) => a.slug !== current.slug && (a as any).category === 'academy');
...
export default async function ArticlePage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  const breadcrumbJsonLd = {
    ...
    itemListElement: [
      ...
      { '@type': 'ListItem', position: 2, name: 'Academy', item: 'https://memedesk.co/academy' },
      { '@type': 'ListItem', position: 3, name: article.headline, item: `https://memedesk.co/academy/${slug}` },
    ],
  };
    ...
    <article className="mx-auto max-w-3xl py-8">
      ...
      {related.length > 0 && (
        <section className="mt-12">
          <h2 className="mb-4 text-xl font-bold text-white">More from Academy</h2>
...
export async function generateStaticParams() {
  return getAllArticles().filter((a) => a.category === 'academy').map((a) => ({ slug: a.slug }));
}
