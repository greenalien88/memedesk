...
export async function generateMetadata({ params }: { params: Promise<{ slug: string }> }): Promise<Metadata> {
  ...
  return {
    ...
    alternates: { canonical: `https://memedesk.co/kol-watch/${slug}` },
    openGraph: {
...
function getRelatedArticles(current: Article, allArticles: Article[], count = 3) {
  const others = allArticles.filter((a) => a.slug !== current.slug && (a as any).category === 'kol-watch');
...
export default async function ArticlePage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  const breadcrumbJsonLd = {
    ...
    itemListElement: [
      ...
      { '@type': 'ListItem', position: 2, name: 'KOL Watch', item: 'https://memedesk.co/kol-watch' },
      { '@type': 'ListItem', position: 3, name: article.headline, item: `https://memedesk.co/kol-watch/${slug}` },
    ],
  };
    ...
    <article className="mx-auto max-w-3xl py-8">
      ...
      {related.length > 0 && (
        <section className="mt-12">
          <h2 className="mb-4 text-xl font-bold text-white">More from KOL Watch</h2>
          <div className="grid gap-4 sm:grid-cols-3">
            {related.map((rel) => (
              <Link
                key={rel.slug}
                href={`/kol-watch/${rel.slug}`}
...
export async function generateStaticParams() {
  return getAllArticles().filter((a) => a.category === 'kol-watch').map((a) => ({ slug: a.slug }));
}
