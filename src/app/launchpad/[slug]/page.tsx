...
export async function generateMetadata({ params }: { params: Promise<{ slug: string }> }): Promise<Metadata> {
  ...
  return {
    ...
    alternates: { canonical: `https://memedesk.co/launchpad/${slug}` },
    openGraph: {
...
function getRelatedArticles(current: Article, allArticles: Article[], count = 3) {
  const others = allArticles.filter((a) => a.slug !== current.slug && (a as any).category === 'launchpad');
...
export default async function ArticlePage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  const breadcrumbJsonLd = {
    ...
    itemListElement: [
      ...
      { '@type': 'ListItem', position: 2, name: 'Launch Pad', item: 'https://memedesk.co/launchpad' },
      { '@type': 'ListItem', position: 3, name: article.headline, item: `https://memedesk.co/launchpad/${slug}` },
    ],
  };
    ...
    <article className="mx-auto max-w-3xl py-8">
      ...
      {related.length > 0 && (
        <section className="mt-12">
          <h2 className="mb-4 text-xl font-bold text-white">More from Launch Pad</h2>
          <div className="grid gap-4 sm:grid-cols-3">
            {related.map((rel) => (
              <Link
                key={rel.slug}
                href={`/launchpad/${rel.slug}`}
...
export async function generateStaticParams() {
  return getAllArticles().filter((a) => a.category === 'launchpad').map((a) => ({ slug: a.slug }));
}
