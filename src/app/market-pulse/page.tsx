...
export default function MarketPulsePage() {
  const filtered = getArticlesByCategory('market-pulse');
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="mt-1 text-sm text-white/60">
              Market sentiment, trend analysis, and the vibe check on crypto.
              What&apos;s hot, what&apos;s not, and where the money&apos;s flowing.
            </p>
          </div>
        </div>
        <p className="mt-4 text-xs text-white/40">{filtered.length} pulse report{filtered.length !== 1 ? 's' : ''} published</p>
      </div>

      {filtered.length === 0 ? (
        <div className="rounded-2xl border border-white/10 bg-white/5 p-10 text-center">
          <p className="text-white/40">No market pulse reports yet. The charts are loading...</p>
        </div>
      ) : (
          ...
          {filtered.map((article: any) => (
            ...
            >
              {article.heroImage && (
                <div className="relative aspect-[16/9] w-full overflow-hidden">
                  ...
                  <div className="absolute bottom-4 left-5">
                    <span className="inline-flex items-center gap-1 rounded-full border border-blue-400/60 bg-blue-400/10 px-2.5 py-0.5 text-xs font-semibold text-blue-300 backdrop-blur">
                      📊 {article.signalLabel}
                    </span>
                  </div>
                </div>
              ...
              <div className="p-5">
                ...
                <h2 className="mt-2 text-lg font-semibold leading-snug group-hover:text-blue-300">{article.headline}</h2>
