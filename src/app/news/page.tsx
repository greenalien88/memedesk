...
export default function NewsPage() {
  const filtered = getArticlesByCategory('news');
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="mt-1 text-sm text-white/60">
              Breaking meme coin stories, KOL calls, exchange listings, and everything
              moving the degen markets right now.
            </p>
          </div>
        </div>
        <p className="mt-4 text-xs text-white/40">{filtered.length} report{filtered.length !== 1 ? 's' : ''} published</p>
      </div>

      {filtered.length === 0 ? (
        <div className="rounded-2xl border border-white/10 bg-white/5 p-10 text-center">
          <p className="text-white/40">No news yet. The timeline is quiet... for now.</p>
        </div>
      ) : (
            ...
            >
              ...
              <div className="p-5">
                <div className="flex flex-wrap items-center gap-3 text-xs text-white/40">
                  ...
                  {(article as any).tokenData && (
                    <>
                      <span>·</span>
                      <span className="text-red-400">${(article as any).tokenData.symbol}</span>
                    </>
                  )}
                </div>
